...
func _go_to_tank_select() -> void:
	...
	# If TankSelect is a separate scene file: get_tree().change_scene_to_file("res://lobby/tank_select.tscn")
	# For simplicity we assume a new scene exists and change to it.
	var path = "res://lobby/tank_select.tscn"
