...
func host(port: int = 7777, max_clients: int = 8) -> void:
	...
	players.clear()
	players[my_id] = {"name": _get_local_name(), "tank": null}
...
func _on_peer_connected(id: int) -> void:
	if is_server:
		# add placeholder for new peer
		players[id] = {"name": "Player_%d" % id, "tank": null}
		rpc("client_update_player_list", players)
...
func server_register_player(player_name: String) -> void:
	...
	players[id] = {"name": player_name, "tank": null}
...
func server_set_tank(tank_name: String) -> void:
	...
	if players.has(id):
		...
		players[id] = {"name": "Player_%d" % id, "tank": tank_name}
...
func server_request_start() -> void:
	...
	for peer_id in peers:
		...
		rpc_id(peer_id, "client_load_scene", "res://artilerry/artilerry_scene.tscn", players)
	# also call locally on the server (host)
	print("[NetworkManager] calling client_load_scene locally for host")
	client_load_scene("res://artilerry/artilerry_scene.tscn", players)
...
func client_apply_damage_to_peer(target_peer_id: int, damage: int, shooter_id: int) -> void:
	...
	var name = "Player_%s" % str(target_peer_id)
	var found: Node = null
	var current = get_tree().get_current_scene()
	if current:
		found = _find_node_recursive(current, name)
	else:
		# fallback: search root children recursively
		var root = get_tree().get_root()
		if root:
			found = _find_node_recursive(root, name)

	if found == null:
		print("[NetworkManager] client_apply_damage_to_peer: target node %s not found" % name)
		return
...
@rpc("call_remote")
func client_spawn_explosion(position: Vector3, shell_type: int) -> void:
	# Spawn explosion effect on clients (visual only)
	var explosion_scene = ResourceLoader.load("res://artilerry/Explosion/explosion.tscn")
